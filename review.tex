...
%DIF DEL _v6_HEAD.tex   Fri Apr 24 15:44:11 2026
%DIF ADD _v6.tex        Fri Apr 24 15:02:12 2026
...
\cite{Ferreira2020}}\hskip0pt%DIFAUXCMD
, we repeated the synthesis experiment using the F3 seismic dataset employed in their original work. The F3 dataset contains 3D post-stack seismic data from the Dutch North Sea, widely used as a benchmark in seismic interpretation and image synthesis. Following the same evaluation protocol, each synthetic image is compared against the original seismic image that served as its template, using MSE, DSSIM, }\DIFaddend and \DIFdelbegin \DIFdel{$3^{rd.}$ quantiles values ($Q1$ and $Q3$), as well as the median values ($median$), as welll as the totals considering all groups. The difference between $Q1$ and $Q3$ are named  Interquartile Range (IQR), and the difference between $max$ and $min$ are named simply as overall range.  
}%DIFDELCMD < 
...
% (Sketch types A-E deleted in this revision)%DIFAUXCMD
%DIFDELCMD < \end{itemize}
%DIFDELCMD < 

%DIFDELCMD < %%%
% GAN-Based Image Synthesis paragraph deleted in this revision
\DIFdel{The core of the method proposed by Ferreira }\emph{\DIFdel{et al.}}%DIFAUXCMD
\DIFdel{~\mbox{%DIFAUXCMD
\cite{Ferreira2020} }\hskip0pt%DIFAUXCMD
employs a conditional GAN (cGAN) architecture based on the pix2pix framework. The }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend generator network takes \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend sketch as input \DIFdelbegin \DIFdel{, }\DIFdelend and learns to produce realistic seismic images \DIFdelbegin \DIFdel{that match the sketches structural layout. The discriminator network evaluates whether the generated images are realistic and properly aligned with the input sketches. The training process involves:
}%DIFDELCMD < \begin{enumerate}
% (Training process steps deleted in this revision)%DIFAUXCMD
%DIFDELCMD < \end{enumerate}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Their proposed GAN-based approach offers flexibility in controlling the output through user-defined sketches, but requires manual intervention to create appropriate sketch inputs. In contrast, }\DIFdelend \DIFaddbegin \DIFadd{matching its structural layout, while the discriminator evaluates the realism and alignment of the generated images. Their approach requires manual sketch creation as a conditional input, whereas }\DIFaddend our proposed VAE-based method with context-oriented texture synthesis operates autonomously, generating both the structural masks and the corresponding seismic textures without requiring manual sketch creation.
...
\DIFdel{~\mbox{%DIFAUXCMD
\cite{Ferreira2020} }\hskip0pt%DIFAUXCMD
five sketch types (A--E) were analysed, and the sketches types D and E were found to produce the best results. In our work, we analyzed four distinct groups, with Group 3 achieving the better, most consistent, performance. The subsequent }\DIFdelend \DIFaddbegin \DIFadd{The following }\DIFaddend subsections present a detailed comparison based on the median values of the measurements \DIFaddbegin \DIFadd{on the F3 dataset}\DIFaddend , which offer a robust measure of each method\DIFaddbegin \DIFadd{'s }\DIFaddend typical performance.

\paragraph{Mean Squared Error (MSE)}

The Mean Squared Error (MSE) measures the pixel-wise distance between two images, with lower values indicating a closer match in intensity. \DIFdelbegin \DIFdel{While the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend sketch-based \DIFdelbegin \DIFdel{method }\DIFdelend \DIFaddbegin \DIFadd{baseline method~\mbox{%DIFAUXCMD
\cite{Ferreira2020} }\hskip0pt%DIFAUXCMD
}\DIFaddend achieved a median MSE of 4712.1 with their best-performing \DIFdelbegin \DIFdel{sketch type, our }\DIFdelend \DIFaddbegin \DIFadd{configuration. Our }\DIFaddend context-oriented approach \DIFdelbegin \DIFdel{yielded }\DIFdelend \DIFaddbegin \DIFadd{achieved }\DIFaddend a median MSE of \DIFdelbegin \DIFdel{542.87. This result is approximately 8.7 times lower}\DIFdelend \DIFaddbegin \textit{[\DIFadd{TODO: F3 result}]}\DIFaddend , demonstrating a substantial improvement in the pixel-wise correspondence between the synthetic and original images.

\paragraph{Structural Similarity Distance (DSSIM)}

The DSSIM measure, derived from SSIM as shown in Eq.~\ref{eq:mse2}, transforms a structural similarity measure into a dissimilarity measure. \DIFdelbegin \DIFdel{In this context, DSSIM values }\DIFdelend \DIFaddbegin \DIFadd{Values }\DIFaddend closer to 0 indicate higher structural similarity between the images.

In Ferreira \emph{et al.}~\cite{Ferreira2020}, the best median DSSIM achieved was 0.39\DIFdelbegin \DIFdel{(with Sketch Type D)}\DIFdelend . Our method \DIFdelbegin \DIFdel{surpassed this benchmark, with Group 2 achieving }\DIFdelend \DIFaddbegin \DIFadd{achieved }\DIFaddend a median DSSIM of \DIFdelbegin \DIFdel{0.2424. This result is significantly better, as }\DIFdelend \DIFaddbegin \textit{[\DIFadd{TODO: F3 result}]}\DIFadd{. Recall that }\DIFaddend a DSSIM value above 0.25 can be interpreted as \DIFdelbegin \DIFdel{"}\DIFdelend \DIFaddbegin \DIFadd{``}\DIFaddend low perceptual similarity.\DIFdelbegin \DIFdel{" While the baseline result falls into this range, our Group 2 median is just below this threshold. Furthermore, Group 2 was not only the top performer in structural quality but also the most consistent, showing the smallest Interquartile Range (IQR) of 0.0080.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lccc}
        ...
        Proposed method & \textit{[TODO]} & \textit{[TODO]} & \textit{[TODO]} \\
        \bottomrule
    \end{tabular}
\end{table}
