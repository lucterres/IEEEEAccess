...
%DIF ADD _v7.tex                                   Sat Jul 18 22:22:34 2026
...
\DIFadd{In summary, among the methods reviewed, }\textbf{Ferreira \emph{et al.}~\cite{Ferreira2020} is the only baseline for which a direct, fair numerical comparison on a common public benchmark (F3 dataset) using the same metrics (MSE, DSSIM, LBP Distance) is possible}\DIFadd{. The remaining methods (Henriques et al., Choi et al., Wang et al.) were evaluated on different datasets, with different metrics, or for different geological targets, precluding a direct numerical comparison. This is a known limitation of the current evaluation, acknowledged candidly, and reflects the broader challenge of the absence of a universally adopted benchmark for seismic image synthesis. Table~}\protect\ref{tab:comparison_overview}\DIFadd{ summarizes the key characteristics of these methods to provide a structured contextual comparison.
}

\DIFaddend \begin{table}[htbp]
    \centering
     \caption{Contextual comparison of seismic image synthesis methods. Direct numerical comparison on a common benchmark is only possible with Ferreira et al.~\cite{Ferreira2020}.}
    ...
    \begin{tabular}{p{2.0cm}p{1.5cm}p{1.6cm}p{1.5cm}}
        ...
        \midrule
        Ferreira et al.~\cite{Ferreira2020}& cGAN (pix2pix) & Salt domes, faults & \textbf{Yes} (F3, same metrics) \\
        Henriques et al.~\cite{Henriques2021}& VAE + CNF & Salt domes & No (different dataset \& metrics) \\
        Choi et al.~\cite{Choi2025}& Cond.\ pix2pix GAN / Diffusion & Fault detection & No (private dataset, downstream metrics) \\
        Wang et al.~\cite{Wang2021}& GAN & Waveforms & No (waveform-level evaluation) \\
        \midrule
...
\noindent\DIFadd{\textbf{Dataset.}} \DIFadd{All experiments use the TGS Salt Identification Challenge dataset~\cite{TGS2023}($101 \times 101$ pixels, 8-bit grayscale; described in Section~}\protect\ref{sec:results}\DIFadd{). A stratified split by salt-coverage fraction (0--10\%, 10--30\%, 30--50\%, $>$50\%) yields }\DIFadd{\textbf{800 real training images}} \DIFadd{and a }\DIFadd{\textbf{fixed test set of 800 real images}} \DIFadd{that is never used during training.
}

\noindent\DIFadd{\textbf{Synthetic pool.}} \DIFadd{The proposed method generates }\DIFadd{\textbf{400 synthetic image--mask pairs}}\DIFadd{: 400 latent vectors $z \sim \mathcal{N}(0,I)$ ($d = 100$) are decoded by the VAE (Section~}\protect\ref{sec:methodology}\DIFadd{) into binary masks, which are then processed by the context-oriented texture synthesis pipeline (Section~}\protect\ref{sec:methodology}\DIFadd{). All synthetic content is derived from newly sampled masks and is independent of any test-set image.
}

\noindent\DIFadd{\textbf{Segmentation model.}} \DIFadd{We adopt the standard U-Net~\cite{Ronneberger2015}with encoder channel progression $1 \to 16 \to 32 \to 64 \to 128$ (four levels; $3 \times 3$ convolutions, padding~1, BatchNorm, ReLU, MaxPool $2\times2$), a 256-channel bottleneck, symmetric decoder with bilinear upsampling and skip connections, and a $1\times1$ output head producing logits.
}

\noindent\DIFadd{\textbf{Training scenarios.}} \DIFadd{Two scenarios are compared under identical hyperparameters (Adam, $\text{lr} = 10^{-4}$, batch~16, up to 50~epochs, early stopping patience~10 on validation IoU, with horizontal and vertical flips applied equally to both scenarios):
}\begin{itemize}
    \item \DIFadd{\textbf{Scenario~A --- Real only:}} \DIFadd{trained on 800 real images.
    }\item \DIFadd{\textbf{Scenario~B --- Real + Synthetic:}} \DIFadd{trained on 800 real + 400 synthetic images (1}{\DIFadd{,}}\DIFadd{200 total).
}\end{itemize}
\DIFadd{Each scenario is repeated with three independent random seeds (42, 123, 456) to estimate variance.
}

\noindent\DIFadd{\textbf{Evaluation metrics.}} \DIFadd{Results are measured on the fixed real test set using IoU (Jaccard index), Dice coefficient, and pixel accuracy. Statistical significance is assessed by a paired Wilcoxon signed-rank test between the per-seed IoU values of both scenarios ($\alpha = 0.05$).
}

\subsubsection{\DIFadd{Results}}

\DIFadd{Table~}\protect\ref{tab:downstream}\DIFadd{ reports mean~$\pm$ standard deviation of IoU and Dice over the three seeds for each scenario.
}

\begin{table}[htbp]
    \centering
    \caption{Downstream segmentation results on the real TGS test set (mean $\pm$ std, 3~seeds). \textcolor{red}{[TODO: replace dashes with experimental results.]}}
    \label{tab:downstream}
    \begin{tabular}{lcc}
        \toprule
        \textbf{Scenario} & \textbf{IoU (mean $\pm$ std)} & \textbf{Dice (mean $\pm$ std)} \\
        \midrule
        A --- Real only ($N{=}800$)         & --- & --- \\
        B --- Real + Synthetic ($N{=}1200$) & --- & --- \\
        \midrule
        $p$-value (A vs.\ B)                & --- & --- \\
        \bottomrule
    \end{tabular}
...
\DIFadd{\textbf{[TODO: Insert analysis of Table~}}\protect\ref{tab:downstream}\DIFadd{\textbf{: absolute IoU/Dice gains, statistical significance, and interpretation of the augmentation benefit in the seismic segmentation context.]}}
...
\textbf{[TODO: Insert IoU $\times N$ curve (Scenario~A vs.\ Scenario~B) and discussion. Expected outcome: the augmentation benefit is more pronounced under data scarcity, consistent with the core motivation of the method.]}
