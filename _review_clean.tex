...
%DIF DEL docs/reviewPacote-submetido-jun/_v6.tex   Thu Jul 16 09:16:23 2026
%DIF ADD _v7.tex                                   Thu Jul 16 14:48:56 2026
...
The salt body dataset from the TGS Salt Identification Challenge is employed for qualitative expert evaluation \DIFaddbegin \DIFadd{and for the ablation study}\DIFaddend . It comprises 2D image slices representing a 3D view of the interior of the Earth obtained through reflection seismology. The data is a set of images chosen randomly in the subsurface, with dimensions of $101 \times 101$ pixels, where each pixel is classified as either salt or sediment \cite{TGS2023}. \DIFaddbegin \DIFadd{Pixel values are stored as 8-bit grayscale integers in the range $[0, 255]$. }\DIFaddend This dataset is used for the assessment of geoscience domain experts to evaluate the visual quality and geological realism of the synthetic seismic images obtained by our proposed scheme\DIFaddbegin \DIFadd{, and also for the ablation experiments described in Section~}\DIFaddend \ref{sec:ablation}.
\paragraph{F3 Seismic Dataset}

The F3 dataset, containing 3D post-stack seismic data from the Dutch North Sea, is used for a quantitative comparison of the quality of our synthetic seismic images with the image quality obtained by a comparable method representing the state-of-the-art proposed by Ferreira \emph{et al.}~\cite{Ferreira2020}. This dataset is widely recognized as a benchmark in seismic interpretation and image synthesis research~\cite{Baroni2018}. \DIFaddbegin \DIFadd{Following the same evaluation protocol as~}\cite{Ferreira2020}\DIFadd{, we extract 2D image patches of $400 \times 400$ pixels from the F3 volume, and pixel values are normalized to the range $[0, 255]$. A total of 600 synthetic images were generated and evaluated in this experiment. }\DIFaddend Using the same dataset\DIFaddbegin \DIFadd{, image dimensions, preprocessing, }\DIFaddend and evaluation protocol as the method in~\cite{Ferreira2020}  ensures a direct and fair comparison based on quantitative measures (MSE, DSSIM, and LBP Distance).
...
\bigskip
\noindent\parbox{0.97\linewidth}{%
\textbf{Experimental setting.} \DIFadd{All ablation experiments in this section are conducted on the \textbf{TGS Salt Identification Challenge dataset} ($101 \times 101$ pixels, 8-bit grayscale), which is distinct from the F3 dataset used for the quantitative comparison in Table~}\DIFaddend \ref{tab:metricsSummary}\DIFaddbegin \DIFadd{.} \DIFadd{As noted in Section~}\DIFaddend \ref{sec:results}\DIFaddbegin \DIFadd{, the smaller image dimensions of the TGS dataset result in MSE values approximately one order of magnitude lower than those in Table~}\DIFaddend \ref{tab:metricsSummary}\DIFaddbegin \DIFadd{ (F3, $400 \times 400$ pixels), because MSE is the mean of squared pixel differences over the entire image area. Consequently, the ablation metrics should be interpreted within this context and are not directly comparable to the F3-based results.}
}
\DIFaddend
% ...existing code...
}
\DIFaddend 
\DIFaddbegin \DIFadd{The }\DIFaddend qualitative evaluation of the synthesized seismic \DIFdelbegin \DIFdel{image samples through a systematic process with the support of geoscientists, who are experts in the field.
Our goal is to evaluate the synthesized samples quality, relevance, and reliability using the knowledge and experience of qualified professionals. Three geoscientists who routinely work in seismic analysis and interpretation participated in this evaluation. We conducted the evaluation by having experts reviewing the seismic images, and selecting different regions types by painting the salt bodies }\DIFdelend \DIFaddbegin \DIFadd{images is being redesigned as a formal blind discrimination experiment, following the protocol described below.
}
...
\DIFadd{Beyond the direct numerical comparison with Ferreira }\emph{\DIFadd{et al.}}\DIFadd{~\cite{Ferreira2020}, it is informative to position the proposed method within the broader landscape of GAN-based }\DIFaddend and \DIFdelbegin \DIFdel{these findings were supported by an }\DIFdelend \DIFaddbegin \DIFadd{diffusion-based seismic image synthesis methods reviewed in Section~}\DIFaddend \ref{sec:related}\DIFaddbegin \DIFadd{.
}
...
\DIFadd{Wang }\emph{\DIFadd{et al.}}\DIFadd{~\cite{Wang2021}(SeismoGen) apply GANs to seismic waveform synthesis and evaluate results at the waveform level; their evaluation framework is incompatible with image-level structural similarity metrics.
}

\DIFadd{In summary, among the methods reviewed, }\textbf{Ferreira \emph{et al.}}\DIFadd{~}\cite{Ferreira2020}\DIFadd{ is the only baseline for which a direct, fair numerical comparison on a common public benchmark (F3 dataset) using the same metrics (MSE, DSSIM, LBP Distance) is possible. The remaining methods (Henriques et al., Choi et al., Wang et al.) were evaluated on different datasets, with different metrics, or for different geological targets, precluding a direct numerical comparison. This is a known limitation of the current evaluation, acknowledged candidly, and reflects the broader challenge of the absence of a universally adopted benchmark for seismic image synthesis. Table~}\DIFaddend \ref{tab:comparison_overview}\DIFaddbegin \DIFadd{ summarizes the key characteristics of these methods to provide a structured contextual comparison.
}
}
\DIFaddend \begin{table}[htbp]
    \centering
     \caption{\DIFadd{Contextual comparison of seismic image synthesis methods. Direct numerical comparison on a common benchmark is only possible with Ferreira et al.~}\cite{Ferreira2020}\DIFadd{.}}
    ...
    \begin{tabular}{p{2.0cm}p{1.5cm}p{1.6cm}p{1.5cm}}
        ...
        \midrule
        \DIFadd{Ferreira et al.~}\cite{Ferreira2020}\DIFadd{} & cGAN (pix2pix) & Salt domes, faults & \textbf{Yes} (F3, same metrics) \\
        \DIFadd{Henriques et al.~}\cite{Henriques2021}\DIFadd{} & VAE + CNF & Salt domes & No (different dataset \& metrics) \\
        \DIFadd{Choi et al.~}\cite{Choi2025}\DIFadd{} & Cond.\ pix2pix GAN / Diffusion & Fault detection & No (private dataset, downstream metrics) \\
        \DIFadd{Wang et al.~}\cite{Wang2021}\DIFadd{} & GAN & Waveforms & No (waveform-level evaluation) \\
        \midrule
...
\DIFaddbegin \noindent\parbox{0.97\linewidth}{%
\textbf{Experimental setting.} \DIFadd{All ablation experiments in this section are conducted on the \textbf{TGS Salt Identification Challenge dataset} ($101 \times 101$ pixels, 8-bit grayscale), which is distinct from the F3 dataset used for the quantitative comparison in Table~}\DIFaddend \ref{tab:metricsSummary}\DIFaddbegin \DIFadd{.As noted in Section~}\DIFaddend \ref{sec:results}\DIFaddbegin \DIFadd{, the smaller image dimensions of the TGS dataset result in MSE values approximately one order of magnitude lower than those in Table~}\DIFaddend \ref{tab:metricsSummary}\DIFaddbegin \DIFadd{ (F3, $400 \times 400$ pixels), because MSE is the mean of squared pixel differences over the entire image area. Consequently, the ablation metrics should be interpreted within this context and are not directly comparable to the F3-based results.}
}
\DIFaddend
