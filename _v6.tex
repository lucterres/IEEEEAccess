...
The baseline method proposed by Ferreira \emph{et al.}~\cite{Ferreira2020} employs a conditional Generative Adversarial Network (cGAN) architecture based on the pix2pix framework to synthesize seismic images from user-provided sketches. The generator network takes a sketch as input and learns to produce realistic seismic images matching its structural layout, while the discriminator evaluates the realism and alignment of the generated images. Their approach requires manual sketch creation as a conditional input, whereas our proposed VAE-based method with context-oriented texture synthesis operates autonomously, generating both the structural masks and the corresponding seismic textures without requiring manual sketch creation.


\subsubsection{Comparative Analysis}

The following subsections present a detailed comparison based on the median values of the measurements on the F3 dataset, which offer a robust measure of each method's typical performance.

\paragraph{Mean Squared Error (MSE)}

The Mean Squared Error (MSE) measures the pixel-wise distance between two images, with lower values indicating a closer match in intensity. The sketch-based baseline method~\cite{Ferreira2020} achieved a median MSE of 4712.1 with their best-performing configuration. Our context-oriented approach achieved a median MSE of 2037.88, demonstrating a substantial improvement in the pixel-wise correspondence between the synthetic and original images.

\paragraph{Structural Similarity Distance (DSSIM)}

The DSSIM measure, derived from SSIM as shown in Eq.~\ref{eq:mse2}, transforms a structural similarity measure into a dissimilarity measure. Values closer to 0 indicate higher structural similarity between the images.
