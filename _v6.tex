...
The quantitative evaluation of the proposed method uses the same measures adopted by Ferreira et al.~\cite{Ferreira2020}, namely MSE, DSSIM, and LBP Distance, enabling a fair comparison with the same metrics. Table~\ref{tab:metricsSummary} presents summary statistics --- minimum ($min$), first quartile ($Q1$), median, third quartile ($Q3$), and maximum ($max$) --- computed over the full set of synthesized seismic image samples. The interquartile range (IQR = $Q3 - Q1$) and the overall range ($max - min$) are used to assess the consistency of each measure across different samples.

Regarding pixel-level fidelity, the global median MSE of 779.19 indicates that the proposed method consistently produces images close to the original templates, with an IQR of 362.28 reflecting the natural variability expected across diverse geological contexts.

In terms of structural similarity, the global median DSSIM of 0.2766 confirms that the synthesized images maintain high structural resemblance to the originals. The relatively narrow spread of the distribution indicates stable performance across the full range of synthesized samples. The box plot in Fig.~\ref{fig:boxplots} further illustrates this distribution.

Regarding texture fidelity, the global median LBP Distance of 0.1640 demonstrates that the proposed method reliably reproduces the characteristic seismic texture patterns found in real images.

\subsection{Comparison with the State-of-the-Art}

To provide a quantitative comparison, we evaluate our proposed method against the sketch-based approach of Ferreira \emph{et al.}~\cite{Ferreira2020}. Our aggregate results are shown in Table~\ref{tab:metricsSummary}, and the results obtained by Ferreira \emph{et al.}~\cite{Ferreira2020} are shown in their original Table~I. Both methodologies measure the similarity between synthetic and original seismic images using three key measures: Mean Squared Error (MSE), Structural Similarity Dissimilarity (DSSIM), and Euclidean Distance based on Local Binary Patterns (LBP Distance). For these measures, lower values signify better performance. Because the two methods employ fundamentally different experimental designs --- Ferreira et al.~\cite{Ferreira2020} report results organized by sketch input complexity, whereas the proposed method produces a single distribution over the full synthesized dataset --- the comparison is made between the global aggregate statistics of the proposed method and the best-reported results of the baseline.

The baseline method proposed by Ferreira \emph{et al.}~\cite{Ferreira2020} employs a conditional GAN (cGAN) architecture based on the pix2pix framework to synthesize seismic images from user-provided sketches. The generator network takes a hand-drawn or computer-generated sketch as input and learns to produce realistic seismic images that match its structural layout, while the discriminator evaluates whether the generated images are realistic and properly aligned with the input sketch. Their approach offers flexibility in controlling the output through user-defined sketches, but requires manual intervention to create appropriate sketch inputs. In contrast, our proposed VAE-based method with context-oriented texture synthesis operates autonomously, generating both the structural masks and the corresponding seismic textures without requiring manual sketch creation.


\subsubsection{Comparative Analysis}

The subsequent subsections present a detailed metric-by-metric comparison, using the global median of the proposed method as a robust measure of typical performance across all synthesized samples.

\paragraph{Mean Squared Error (MSE)}

The Mean Squared Error (MSE) measures the pixel-wise distance between two images, with lower values indicating a closer match in intensity. While the sketch-based method achieved a best median MSE of 4712.1, the proposed context-oriented approach achieves a global median MSE of 779.19 over all synthesized samples. This result is approximately 6 times lower, demonstrating a substantial improvement in pixel-level correspondence between the synthetic and original images.
...
In Ferreira \emph{et al.}~\cite{Ferreira2020}, the best median DSSIM achieved was 0.39. The proposed method achieves a global median DSSIM of 0.2766 over all synthesized samples, representing a clear improvement of approximately 0.11 absolute units. The entire observed distribution of our DSSIM values remains well below the baseline best of 0.39, with an IQR of 0.0303 confirming consistent structural quality across diverse samples.
...
The baseline work in Ferreira \emph{et al.}~\cite{Ferreira2020} reported a best median LBP Distance of 0.17. The proposed method achieves a global median LBP Distance of 0.1640 over all synthesized samples, a lower value indicating higher overall texture fidelity to the original seismic images. Notably, the lower end of the distribution (min = 0.04, Q1 = 0.1043) shows that a substantial portion of the synthesized samples achieve texture similarity well below the baseline, demonstrating the capacity of the proposed method to produce images with high textural fidelity across diverse geological contexts.
...
It is important to note that quantitative measures may not always align with qualitative human assessment. For instance, the baseline study observed that their best-quantitative-scoring configuration did not always produce the most visually realistic images. In our work, we supplemented our quantitative findings with a detailed qualitative analysis by domain experts. The experts evaluations concluded that the synthetic images were virtually indistinguishable from the real ones, a finding supported by an F1-score difference of less than 2\%.


\begin{figure}[htbp]
	\centering
	\includegraphics[width=1\linewidth]{images/terre11}
	\caption{Distribution of DSSIM values across all synthesized seismic image samples.}
	\label{fig:boxplots}
\end{figure}
...
\begin{table}[htbp]
    \centering
    \caption{Summary statistics for MSE, DSSIM, and LBP Distance across all synthesized seismic image samples.}
