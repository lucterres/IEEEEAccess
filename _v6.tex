...
\begin{itemize}
    \item \textbf{Experiment}: Generate two sets of 100 masks each: (1) using the trained VAE by sampling from the latent space, and (2) using simple geometric transformations (\emph{e.g.}, rotation, scaling, horizontal/vertical flipping, elastic deformation) applied to existing ground-truth masks from the training set.
