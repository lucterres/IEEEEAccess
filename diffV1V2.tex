...
%DIF DEL X_accessV1.tex   Wed Sep  3 10:52:40 2025
%DIF ADD accessv2.tex     Tue Sep  9 21:33:32 2025
%\usepackage{cite}
...
\DIFaddbegin \DIFadd{In 2020, R. S. Ferreira et al. \mbox{%DIFAUXCMD
...
\DIFaddend \section{Proposed Methodology}

The Context-Oriented Seismic Image Synthesis methodology combines a deep learning model \DIFdelbegin \DIFdel{(VAE ) }\DIFdelend \DIFaddbegin \DIFadd{based on VAE }\DIFaddend with a texture synthesis algorithm. It generates seismic image samples for data augmentation within a specific context mask, as outlined below. The convolutional neural network is a variational auto-encoder model (VAE) that generates salt body masks corresponding to a given salt geometry. Using the generated mask geometry, a new image is synthesized using a non-parametric texture synthesis algorithm. The proposed method focuses on context zones and on the characteristics of seismic images with salt domes. %A schema of the proposed method is presented in Algorithm.  ~\ref{alg:alg1}.
...
\begin{enumerate}
    \item \textbf{Qualitative \DIFdelbegin \DIFdel{Comparison }\DIFdelend \DIFaddbegin \DIFadd{Evaluation }\DIFaddend by Experts}: Experts \DIFdelbegin \DIFdel{evaluate seismic samples by trying }\DIFdelend \DIFaddbegin \DIFadd{try }\DIFaddend to identify the portions of salt dome structures inside \DIFdelbegin \DIFdel{them}\DIFdelend \DIFaddbegin \DIFadd{the seismic samples}\DIFaddend . The expert produces a mask label that will be compared with the ground truth \DIFaddbegin \DIFadd{mask}\DIFaddend . Quantitative metrics, such as the F1 score, are used to \DIFdelbegin \DIFdel{measure the }\DIFdelend \DIFaddbegin \DIFadd{evaluate the difference between the }\DIFaddend expert prediction and the ground truth. 

    \item \textbf{\DIFdelbegin \DIFdel{Comparing results }\DIFdelend \DIFaddbegin \DIFadd{Quantitative Comparison }\DIFaddend with \DIFdelbegin \DIFdel{related works}\DIFdelend \DIFaddbegin \DIFadd{Related Work}\DIFaddend }:
    \DIFaddbegin \DIFadd{A very similar work }\DIFaddend In “Generating Sketch-Based Synthetic Seismic Images With Generative Adversarial Networks” \cite{ref19}, use the metrics mean squared error (MSE), structural similarity (SSIM) and Euclidean distance based on the Local Binary Pattern (LBP) to measure texture attributes between original seismic sketches and the samples synthesized by the networks. \DIFaddbegin \DIFadd{We will evaluate the samples generated by our proposed method with the same metrics and compare the results.
...
\DIFdelbegin \subsection{\DIFdel{Qualitative Comparison by Experts}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
...
\DIFaddbegin \subsection{\DIFadd{Qualitative Evaluation by Experts}}

\DIFaddend The qualitative evaluation of the samples was conducted through a systematic and rigorous process with the support of \DIFaddbegin \DIFadd{geoscientists, }\DIFaddend experts in the field. The goal was to verify the synthesized samples' quality, relevance, and reliability using the knowledge and experience of qualified professionals. Three \DIFdelbegin \DIFdel{geology and geophysics experts }\DIFdelend \DIFaddbegin \DIFadd{geoscientists }\DIFaddend who routinely work in seismic interpretation participated in this evaluation. The evaluation was conducted by reviewing \DIFdelbegin \DIFdel{a }\DIFdelend seismic image and selecting the regions by painting the salt bodies using a graphics computer program. The area painted by the expert is then compared with the jig established for the corresponding saline body of the image. First, the specialist's accuracy was assessed by identifying salt bodies on twenty original images from the dataset. Then, the selection of the areas is performed on a set of thirty random synthetic images. The specialists repeat the process with the same images. In addition, experts are warned that some images are added for control without any saline body.
...
\subsection{\DIFdelbegin \DIFdel{Comparing }\DIFdelend \DIFaddbegin \DIFadd{Quantitative comparison }\DIFaddend with Related \DIFdelbegin \DIFdel{Work}\DIFdelend \DIFaddbegin \DIFadd{Works}\DIFaddend }
\DIFaddbegin \vspace{5mm} { \DIFadd{\textcolor{red} }{\textbf{\DIFadd{DOING with latexmarkup again}}}}  \vspace{5mm}
\DIFaddend 
...
\DIFdelbegin \DIFdel{received }\DIFdelend \DIFaddbegin \DIFadd{Received }\DIFaddend the B.S. degree in \DIFdelbegin \DIFdel{Computational }\DIFdelend \DIFaddbegin \DIFadd{Computer }\DIFaddend Science from the \DIFdelbegin \DIFdel{Universidade Federal do }\DIFdelend \DIFaddbegin \DIFadd{Federal University of }\DIFaddend Rio
Grande do Sul \DIFaddbegin \DIFadd{(UFRGS), Brazil}\DIFaddend , \DIFdelbegin \DIFdel{Porto Alegre - RS, }\DIFdelend in 1996. In 2010 received the M.S. \DIFdelbegin \DIFdel{and the Ph.D. degree in
...
inventions. }\DIFdelend \DIFaddbegin \DIFadd{degree in
