...
\begin{abstract}
Advanced remote sensing and seismic image analysis rely on training robust machine learning models, which require large annotated datasets. However, obtaining such datasets is challenging in applications like offshore oil exploration, where expert annotation is costly and time-consuming. This work proposes a context-oriented data augmentation scheme for generating synthetic seismic images of salt dome structures with pixel-level mask labels. The proposed approach uses Variational Autoencoders (VAE) to learn and generate realistic salt body geometries, which are combined with a context-aware non-parametric texture synthesis method, to create seismic textures across three distinct zones: salt bodies, conventional rocks, and their boundaries. Using the TGS Salt Identification Challenge dataset as training data, several generated synthetic samples were evaluated by three geoscientists through a formal blind discrimination experiment, in which real and synthetic images were presented in a single randomized interleaved sequence. The evaluators classified each image as real or synthetic and performed salt-body segmentation. Classification accuracy was 63.7\% ($n=270$; one-sided binomial test: $p < 0.001$), significantly above chance, indicating that the synthetic images are sufficiently realistic to challenge expert discrimination. Segmentation F1-scores were $0.849 \pm 0.065$ for real images and $0.840 \pm 0.067$ for synthetic images, confirming that the synthetic samples support expert salt-body interpretation at a level comparable to real images.
\end{abstract}
...
We use the generated masks as contexts for the texture synthesis process. Once the boundary of a salt body is identified, its interior is assumed to correspond to the salt region. The VAE operates on binary salt-body masks. The original $101 \times 101$ masks are resized to $64 \times 64$ before being used as inputs, yielding an input dimension of $1 \times 64 \times 64$. The encoder consists of three \texttt{Conv2d} layers with channel progression $1 \rightarrow 128 \rightarrow 256 \rightarrow 512$, kernel size $3$, stride $2$, and padding $1$. The resulting $512 \times 8 \times 8$ feature map is flattened and passed to two independent linear layers that estimate the latent mean $\mu$ and log-variance $\log\sigma^2$, both with dimensionality $d=100$. Latent codes are sampled using $z=\mu+\sigma\odot\epsilon$, where $\sigma=\exp(\frac{1}{2}\log\sigma^2)$ and $\epsilon\sim\mathcal{N}(0,\mathrm{I})$. The decoder is an MLP with layer dimensions $100\rightarrow256\rightarrow512\rightarrow1024\rightarrow4096$. Its 4096-dimensional output is reshaped into a $1\times64\times64$ mask and passed through a sigmoid activation.
...
\textbf{1. Neighborhood:} For each pixel $p$ in the image to be synthesized, a region around $p$ already filled with known texture pixel values is defined (namely, a neighborhood $N(p)$). In our implementation, the neighborhood window is $11 \times 11$ pixels, following the default configuration of the non-parametric synthesis algorithm of Efros and Leung~\cite{Efros1999};
...
\end{equation}
where $\sigma$ is a temperature parameter that controls the influence of distances in the pixel selection probability, and $q'$ denotes all candidate pixels in the example texture image. In our implementation, $\sigma$ is computed dynamically as $\sigma = \text{kernel\_size} / 6.4 \approx 1.72$, following the reference implementation of Efros and Leung~\cite{Efros1999};
...
\PARstart{T}{his} section presents and discusses the experimental results to illustrate the effectiveness of the proposed approach. The experimental evaluation encompasses a qualitative assessment performed by domain experts and a quantitative comparison with the state-of-the-art method in~\cite{Ferreira2020}. This dual evaluation strategy follows the criteria below:

\begin{enumerate}
    \item \textbf{Qualitative Evaluation by Experts — Blind Discrimination Experiment}: To rigorously assess the perceptual realism of the synthetic images, a formal blind discrimination experiment was conducted. Real and synthetic seismic images were presented to three expert geoscientists in a single randomized, interleaved sequence. For each image, experts classified it as real or synthetic and performed salt-body segmentation. Overall classification accuracy was $63.7\%$ ($n = 270$; one-sided binomial test: $p < 0.001$), significantly above chance, indicating that the synthetic images are sufficiently realistic to challenge expert discrimination. Segmentation F1-scores were $0.849 \pm 0.065$ for real images and $0.840 \pm 0.067$ for synthetic images, confirming that the synthetic samples support expert salt-body interpretation at a level comparable to real images.
    
    \item \textbf{Quantitative Comparison with the State-of-the-Art}:
    
    The quantitative comparison is performed with the method in~\cite{Ferreira2020} because it is representative of the state of the art and addresses the same problem as our work. To enable a direct comparison, we reproduced the synthesis experiment using the F3 seismic dataset employed in~\cite{Ferreira2020}. This quantitative comparison is based on three measures: mean squared error (MSE), structural similarity index (SSIM), and the Euclidean distance between Local Binary Pattern (LBP) histograms for texture attribute assessment. These measures quantify the similarity between the original seismic images and the synthetic samples generated by the chosen synthesis method.
\end{enumerate}
...
The salt body dataset from the TGS Salt Identification Challenge is employed for qualitative expert evaluation and for the ablation study. It comprises 2D image slices representing a 3D view of the interior of the Earth obtained through reflection seismology. The data is a set of images chosen randomly in the subsurface, with dimensions of $101 \times 101$ pixels, where each pixel is classified as either salt or sediment \cite{TGS2023}. Pixel values are stored as 8-bit grayscale integers in the range $[0, 255]$. This dataset is used for the assessment of geoscience domain experts to evaluate the visual quality and geological realism of the synthetic seismic images obtained by our proposed scheme, and also for the ablation experiments described in Section~\ref{sec:ablation}.

\paragraph{F3 Seismic Dataset}

The F3 dataset, containing 3D post-stack seismic data from the Dutch North Sea, is used for a quantitative comparison of our synthetic seismic images with the results obtained by the state-of-the-art method in~\cite{Ferreira2020}. This dataset is widely recognized as a benchmark in seismic interpretation and image synthesis research~\cite{Baroni2018}. Following the same evaluation protocol as~\cite{Ferreira2020}, we extract 2D image patches of $400 \times 400$ pixels from the F3 volume, and pixel values are normalized to the range $[0, 255]$. A total of 600 synthetic images were generated and evaluated in this experiment. Using the same dataset, image dimensions, preprocessing, and evaluation protocol as~\cite{Ferreira2020} ensures a direct and fair comparison based on quantitative measures (MSE, DSSIM, and LBP Distance).

\noindent\textbf{Note on experimental settings and numerical ranges.} The two experimental contexts in this work --- the quantitative comparison with the method in~\cite{Ferreira2020} and the ablation study (Section~\ref{sec:ablation}) --- use \emph{different datasets, image dimensions, and sample sizes}, and are therefore not directly numerically comparable. The quantitative comparison with the method in~\cite{Ferreira2020} is conducted on the F3 dataset using $400 \times 400$ pixel images ($N = 600$ synthetic images), producing MSE values in the range of approximately 3,700--6,700. The ablation study is conducted on the TGS dataset using $101 \times 101$ pixel images ($N = 110$ images for the texture ablation; $N = 200$ masks for the VAE ablation), producing MSE values in the range of approximately 500--850. The substantially smaller image area ($101^2 = 10{,}201$ pixels vs.\ $400^2 = 160{,}000$ pixels) accounts for the order-of-magnitude difference in absolute MSE values between the two experimental contexts, since MSE is computed as the mean of squared pixel differences over all pixels of each image pair. DSSIM and LBP Distance values are less sensitive to image size but are still influenced by the different image content characteristics of the two datasets. Results from each context should therefore be interpreted independently.
...
The qualitative evaluation of the synthesized seismic images was conducted as a formal blind discrimination experiment, following the protocol described below.

\subsubsection{Blind Discrimination Experiment Protocol}

In this evaluation, real images from the TGS Salt Identification Challenge dataset and synthetic seismic images, such as those illustrated in Fig.~\ref{fig:synthetic_samples}, were presented to expert geoscientists in a single, randomized, interleaved sequence. The stimulus set consisted of \textbf{45 real images with salt}, \textbf{45 synthetic images with salt}, and \textbf{10 control images without any saline body} (100 images total). Experts were \emph{not} informed of the origin of each image prior to evaluation. For each image, the experts were asked to:
\begin{enumerate}
    \item Classify the image as \emph{real} or \emph{synthetic};
    \item Identify and segment the salt body regions by marking them in a digital image viewer.
\end{enumerate}
The segmentation masks produced by the experts are compared against the ground-truth masks using the precision, recall, and F1-score measures described in Section~\ref{eval:meas:sect}. The evaluation involves three geoscientists who routinely work in seismic analysis and interpretation. The identity and sequence of images are fully randomized across participants to avoid order effects.

\subsubsection{Blind Discrimination Results}

Table~\ref{tab:blind_classification} reports the classification accuracy per expert and overall.
The corresponding confusion matrices are presented in Fig.~\ref{fig:discrimination_confusion_matrix_per_evaluator} for the individual evaluators and in Fig.~\ref{fig:discrimination_confusion_matrix_pooled} for all evaluators combined. Each cell reports the number of classifications and the percentage within the corresponding ground-truth class.

\begin{figure*}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{images/terre12.png}
    \caption{Confusion Matrix - Blind Origin Discrimination (Real vs. Synthetic) (control images excluded).}
    \label{fig:discrimination_confusion_matrix_per_evaluator}
\end{figure*}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.72\linewidth]{images/terre13.png}
    \caption{Confusion Matrix - Blind Origin Discrimination (Real vs. Synthetic) All evaluators combined (control images excluded).}
    \label{fig:discrimination_confusion_matrix_pooled}
\end{figure}

\begin{table}[htbp]
    \centering
    \caption{Blind discrimination accuracy: expert classification of real vs.\ synthetic seismic images (45 real + 45 synthetic per evaluator; control images excluded).}
    \label{tab:blind_classification}
    \footnotesize
    \begin{tabular}{lcccc}
        \toprule
        \textbf{Evaluator} & \textbf{Acc. real (\%)} & \textbf{Acc. synth. (\%)} & \textbf{Overall acc. (\%)} \\
        \midrule
        Expert 1 & 62.2 & 66.7 & 64.4 \\
        Expert 2 & 66.7 & 68.9 & 67.8 \\
        Expert 3 & 60.0 & 57.8 & 58.9 \\
        \midrule
        \textbf{All} & \textbf{63.0} & \textbf{64.4} & \textbf{63.7} \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[htbp]
    \centering
    \caption{Expert segmentation quality on real and synthetic images (mean $\pm$ std F1-score, $n=45$ per type per evaluator).}
    \label{tab:blind_segmentation}
    \footnotesize
    \begin{tabular}{lccc}
        \toprule
        \textbf{Evaluator} & \textbf{F1 — Real} & \textbf{F1 — Synthetic} & $\bm{\Delta}$ \\
        \midrule
        Expert 1 & $0.874 \pm 0.052$ & $0.874 \pm 0.049$ & $+0.000$ \\
        Expert 2 & $0.872 \pm 0.054$ & $0.870 \pm 0.043$ & $-0.002$ \\
        Expert 3 & $0.801 \pm 0.061$ & $0.777 \pm 0.058$ & $-0.024$ \\
        \midrule
        \textbf{All} & $\mathbf{0.849 \pm 0.065}$ & $\mathbf{0.840 \pm 0.067}$ & $\mathbf{-0.009}$ \\
        \bottomrule
    \end{tabular}
\end{table}

The overall classification accuracy of $63.7\%$ ($n = 270$) is statistically significant above chance (one-sided exact binomial test: $k = 172$, $p = 0.000004$; one-sided 95\% CI: $p > 0.586$). This indicates that the synthetic images are sufficiently realistic to challenge expert discrimination, but not fully undistinguishable from real images. The confusion between classes is near-symmetric: real images were misclassified as synthetic in 37.0\% of trials, and synthetic images as real in 35.6\%, suggesting that neither class was consistently easier to identify.

To assess whether the two error directions were systematically unbalanced, we applied an exact McNemar test to the discordant classifications. In the pooled dataset, the confusion matrix contained 50 real-to-synthetic errors and 48 synthetic-to-real errors, out of 98 discordant classifications. The two-sided exact test was not significant ($p = 0.9196$), providing no evidence of a relevant directional interpretation bias. This analysis evaluates asymmetry between the two error types; it does not constitute a fairness analysis across demographic or independent evaluator groups.

All three experts correctly identified all ten control images (0\% false-positive rate for salt presence in controls), confirming that the evaluators reliably distinguished images with salt from those without.

Regarding segmentation quality, the mean F1-score difference between real and synthetic images was $-0.009$ (Table~\ref{tab:blind_segmentation}), indicating that the synthetic images support expert salt-body segmentation at a level comparable to real images. These results together confirm that the proposed method produces geologically plausible and visually convincing synthetic seismic images that are comparable to real ones for salt-body interpretation.

\subsection{Quantitative Evaluation}

In order to make a direct and fair comparison with the results reported in~\cite{Ferreira2020}, we repeated the corresponding synthesis experiment using the F3 seismic dataset employed in that study. The F3 dataset contains 3D post-stack seismic data from the Dutch North Sea and is widely used as a benchmark in seismic interpretation and image synthesis~\cite{Baroni2018}. Following the same evaluation protocol, each synthetic image is compared against the original seismic image that served as its template, using MSE, DSSIM, and LBP Distance as described in Section~\ref{eval:meas:sect}. The quantitative results obtained by our proposed method on the F3 dataset are presented in Table~\ref{tab:metricsSummary} and discussed in the comparison below.
...
The sketch-based method~\cite{Ferreira2020} achieved a median MSE of $4712.1$ using its best-performing configuration. Our proposed context-oriented approach achieved a median MSE of $3926.2$, corresponding to an absolute reduction of approximately $785.9$ ($\approx 16.7\%$) in pixel-wise error between the synthetic and original images.

The study in~\cite{Ferreira2020} reported a median DSSIM of $0.3978$ using its best-performing configuration. Our proposed context-oriented approach achieved a median DSSIM of $0.3891$, corresponding to an absolute reduction of approximately $0.0087$ ($\approx 2.2\%$). A paired Wilcoxon signed-rank test between the two methods would be the appropriate statistical procedure to assess significance; however, the sample-level DSSIM values reported in~\cite{Ferreira2020} were not publicly released, so only their aggregated statistics (minimum, median, and maximum) are available for comparison. This precludes any formal inter-method significance test for DSSIM. The DSSIM result indicates that \emph{both methods operate in a similar structural-similarity regime}, with a modest advantage for the proposed approach --- a finding that supports the argument that the structural layout of the synthesized images is preserved regardless of the synthesis strategy, while the more pronounced differentiators are captured by MSE and LBP Distance.

The work reported in~\cite{Ferreira2020} had the lowest median LBP Distance value of $0.1714$. In contrast, our proposed method achieved a median LBP Distance of $0.1500$, a reduction of approximately $12.5\%$, indicating that our method generates textures with substantially higher fidelity to the original images in the local binary pattern feature space.

Overall, our proposed context-oriented synthesis approach obtains better quantitative performance across all three measures than the sketch-based method described in~\cite{Ferreira2020}. The improvement is most pronounced in MSE ($\approx 16.7\%$ reduction: from $4712.1$ to $3926.2$) and LBP Distance ($\approx 12.5\%$ reduction: from $0.1714$ to $0.1500$), which together suggest meaningful gains in pixel-level accuracy and textural fidelity. The DSSIM improvement is more modest ($\approx 2.2\%$ reduction: from $0.3978$ to $0.3891$); we do not claim statistical significance for it because the sample-level DSSIM values reported in~\cite{Ferreira2020} were not publicly released, but the direction of the difference is consistent with the gains observed in the other two measures.

\subsubsection{Contextual Comparison with GAN-based and Diffusion-based Methods}

Beyond the direct numerical comparison with the method in~\cite{Ferreira2020}, it is informative to position the proposed method within the broader landscape of GAN-based and diffusion-based seismic image synthesis methods reviewed in Section~\ref{sec:related}.

The method in~\cite{Henriques2021} also uses a VAE for mask generation and is therefore the most closely related architectural baseline. However, that method evaluates quality only through downstream segmentation performance on a different (non-public) synthetic dataset and does not report MSE, DSSIM, or LBP Distance on a public benchmark; a direct numerical comparison is therefore not possible. From an architectural perspective, the key distinction is that its Conditional Normalizing Flow (CNF) texture stage requires training on 24,872 annotated patch pairs, while the proposed method's texture synthesis stage is non-parametric and training-free. This design choice makes the proposed method more data-efficient and directly applicable in settings where only a small number of annotated seismic images are available.

The studies in~\cite{Choi2021} and~\cite{Choi2025} evaluate two conditional generative approaches for seismic data synthesis: a conditional pix2pix GAN and, in a follow-up work, a conditional diffusion model. Both approaches target fault detection and are evaluated on a private dataset using downstream segmentation metrics (F1-score, IoU), not image-level similarity measures (MSE, DSSIM, LBP Distance); a direct numerical comparison is therefore also not feasible. From a modeling perspective, the conditional diffusion model in~\cite{Choi2025} requires hundreds of denoising steps at inference time and large annotated training sets, in contrast to the single-pass generation and training-free texture synthesis of the proposed approach.

The SeismoGen method in~\cite{Wang2021} applies GANs to seismic waveform synthesis and evaluates results at the waveform level; its evaluation framework is incompatible with image-level structural similarity metrics.

In summary, among the methods reviewed, the approach in~\cite{Ferreira2020} is the only baseline for which a direct, fair numerical comparison on a common public benchmark (F3 dataset) using the same metrics (MSE, DSSIM, LBP Distance) is possible. The remaining methods were evaluated on different datasets, with different metrics, or for different geological targets, precluding a direct numerical comparison. We acknowledge this as a known limitation of the current evaluation, reflecting the broader challenge of the absence of a universally adopted benchmark for seismic image synthesis. Table~\ref{tab:comparison_overview} summarizes the key characteristics of these methods to provide a structured contextual comparison.

\begin{table}[htbp]
    \centering
    \caption{Contextual comparison of seismic image synthesis methods. Direct numerical comparison on a common benchmark is only possible with the method in~\cite{Ferreira2020}.}
    ...
    \begin{tabular}{p{2.0cm}p{1.5cm}p{1.6cm}p{1.5cm}}
        ...
        \midrule
        Ferreira et al. & cGAN (pix2pix) & Salt domes, faults & \textbf{Yes} (F3, same metrics) \\
        ...
        Wang et al. & GAN & Waveform aug. & No (waveform-level evaluation) \\
        \midrule
...
\noindent\textbf{Dataset.} All experiments use the TGS Salt Identification Challenge dataset~\cite{TGS2023} ($101 \times 101$ pixels, 8-bit grayscale; described in Section~\ref{sec:results}). A stratified split by salt-coverage fraction (0--10\%, 10--30\%, 30--50\%, $>$50\%) yields \textbf{1{,}200 real training images} and a \textbf{fixed test set of 800 real images} that is never used during training.

\noindent\textbf{Synthetic pool.} The proposed method generates \textbf{1{,}200 synthetic image--mask pairs}: 1{,}200 latent vectors $z \sim \mathcal{N}(0,I)$ ($d = 100$) are decoded by the VAE (Section~\ref{sec:methodology}) into binary masks, which are then processed by the context-oriented texture synthesis pipeline (Section~\ref{sec:methodology}). All synthetic content is derived from newly sampled masks and is independent of any test-set image.
...
\noindent\textbf{Experimental setting.} All ablation experiments in this section are conducted on the \textbf{TGS Salt Identification Challenge dataset} ($101 \times 101$ pixels, 8-bit grayscale), which is distinct from the F3 dataset used for the quantitative comparison in Table~\ref{tab:metricsSummary}. As noted in Section~\ref{sec:results}, the smaller image dimensions of the TGS dataset result in MSE values approximately one order of magnitude lower than those in Table~\ref{tab:metricsSummary} (F3, $400 \times 400$ pixels), because MSE is the mean of squared pixel differences over the entire image area. Consequently, the ablation metrics should be interpreted within this context and are not directly comparable to the F3-based results.
...
The comprehensive evaluation framework, combining expert qualitative assessment with rigorous quantitative measures, validates the effectiveness of our methodology. The formal blind discrimination experiment — in which three expert geoscientists classified 45 real and 45 synthetic seismic images in a single randomized interleaved sequence — yielded an overall classification accuracy of $63.7\%$ ($p < 0.001$), confirming that the synthetic images are highly realistic, though not fully indistinguishable from real images. Segmentation F1-scores were comparable between real ($0.849 \pm 0.065$) and synthetic ($0.840 \pm 0.067$) images, supporting the conclusion that the proposed method produces geologically plausible and visually convincing synthetic samples that are suitable for salt-body interpretation. Quantitatively, our proposed method outperforms the state-of-the-art sketch-based GAN approach on the F3 dataset, considering all three measures proposed in~\cite{Ferreira2020} (MSE, DSSIM, and LBP Distance).
...
\bibitem{Choi2021}
W.~Choi and S.~Pyun, ``Synthetic training data generation for fault detection based on deep learning,'' \emph{Geophysics and Geophysical Exploration}, vol. 24, no. 3, pp. 89--97, 2021, doi: 10.7582/GGE.2021.24.3.089.
...
\bibitem{pix2pix2017}
P.~Isola, J.-Y.~Zhu, T.~Zhou, and A.~A.~Efros, ``Image-to-image translation with conditional adversarial networks,'' in \emph{2017 IEEE Conference on Computer Vision and Pattern Recognition (CVPR)}, 2017, pp. 1125--1134, doi: 10.1109/CVPR.2017.632.
