...
\noindent\textbf{Dataset.} All experiments use the TGS Salt Identification Challenge dataset~\cite{TGS2023} ($101 \times 101$ pixels, 8-bit grayscale; described in Section~\ref{sec:results}). A stratified split by salt-coverage fraction (0--10\%, 10--30\%, 30--50\%, $>$50\%) yields \textbf{800 real training images} and a \textbf{fixed test set of 800 real images} that is never used during training.

\noindent\textbf{Synthetic pool.} The proposed method generates \textbf{400 synthetic image--mask pairs}: 400 latent vectors $z \sim \mathcal{N}(0,I)$ ($d = 100$) are decoded by the VAE (Section~\ref{sec:methodology}) into binary masks, which are then processed by the context-oriented texture synthesis pipeline (Section~\ref{sec:methodology}). All synthetic content is derived from newly sampled masks and is independent of any test-set image.
...
\begin{itemize}
    \item \textbf{Scenario~A --- Real only:} trained on 800 real images.
    \item \textbf{Scenario~B --- Real + Synthetic:} trained on 800 real + 400 synthetic images (1{,}200 total).
\end{itemize}
Each scenario is repeated with three independent random seeds (42, 123, 456) to estimate variance.

\noindent\textbf{Evaluation metrics.} Results are measured on the fixed real test set using IoU (Jaccard index), Dice coefficient, and pixel accuracy. Statistical significance is assessed by a paired Wilcoxon signed-rank test between the per-seed IoU values of both scenarios ($\alpha = 0.05$).

\subsubsection{Results}

Table~\ref{tab:downstream} reports mean~$\pm$ standard deviation of IoU and Dice over the three seeds for each scenario.
...
\textcolor{red}{\textbf{[TODO: Insert analysis of Table~\ref{tab:downstream}: absolute IoU/Dice gains, statistical significance, and interpretation of the augmentation benefit in the seismic segmentation context.]}}
