...
In this evaluation, real and synthetic seismic images from the TGS Salt Identification Challenge dataset are presented to expert geoscientists in a single, randomized, interleaved sequence. The stimulus set comprised \textbf{45 real images with salt}, \textbf{45 synthetic images with salt}, and \textbf{10 control images without any saline body} (100 images total). Experts are \emph{not} informed of the origin of each image prior to evaluation. For each image, the experts are asked to:
\begin{enumerate}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lccc}
        \toprule
        \textbf{Evaluator} & \textbf{F1 — Real} & \textbf{F1 — Synthetic} & \textbf{$\Delta$} \\
        \midrule
