...
Ferreira \emph{et al.}~\cite{Ferreira2020} reported a median DSSIM of $0.3978$ using its best-performing configuration. Our proposed context-oriented approach achieved a median DSSIM of $0.3891$, corresponding to an absolute reduction of approximately $0.0087$ ($\approx 2.2\%$). A paired Wilcoxon signed-rank test between the two methods would be the appropriate statistical procedure to assess significance; however, the sample-level DSSIM values of Ferreira \emph{et al.}~\cite{Ferreira2020} were not publicly released, so only their aggregated statistics (minimum, median, and maximum) are available for comparison. This precludes any formal inter-method significance test for DSSIM. The DSSIM result indicates that \emph{both methods operate in a similar structural-similarity regime}, with a modest advantage for the proposed approach --- a finding that supports the argument that the structural layout of the synthesized images is preserved regardless of the synthesis strategy, while the more pronounced differentiators are captured by MSE and LBP Distance.

The work in Ferreira \emph{et al.}~\cite{Ferreira2020} reported the lowest median LBP Distance value of $0.1714$. In contrast, our proposed method achieved a median LBP Distance of $0.1500$, a reduction of approximately $12.5\%$, indicating that our method generates textures with substantially higher fidelity to the original images in the local binary pattern feature space.

Overall, our proposed context-oriented synthesis approach obtains better quantitative performance across all three measures when compared to the sketch-based method of Ferreira \emph{et al.}~\cite{Ferreira2020}. The improvement is most pronounced in MSE ($\approx 16.7\%$ reduction: from $4712.1$ to $3926.2$) and LBP Distance ($\approx 12.5\%$ reduction: from $0.1714$ to $0.1500$), which together suggest meaningful gains in pixel-level accuracy and textural fidelity. The DSSIM improvement is more modest ($\approx 2.2\%$ reduction: from $0.3978$ to $0.3891$); we do not claim statistical significance for it because the sample-level DSSIM values of Ferreira \emph{et al.}~\cite{Ferreira2020} were not publicly released, but the direction of the difference is consistent with the gains observed in the other two measures.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{2.0cm}p{1.5cm}p{1.6cm}p{1.5cm}}
        ...
        Choi et al.~\cite{Choi2021,Choi2025} & Cond.\ pix2pix GAN + Diffusion & Fault detection & No (private dataset, downstream metrics) \\
        Wang et al.~\cite{Wang2021} & GAN & Waveform aug. & No (waveform-level evaluation) \\
        \midrule
