...
%DIF ADD X_accessV1.tex   Wed Sep  3 10:52:40 2025
%\usepackage{cite}
...
\title{Context-oriented Synthesis \DIFaddbegin \DIFadd{of Salt Domes }\DIFaddend Labeled Seismic Images}
...
\section{Proposed \DIFdelbegin \DIFdel{Context-Oriented Seismic Image Synthesis }\DIFdelend Methodology}

The \DIFdelbegin \DIFdel{proposed }\DIFdelend \DIFaddbegin \DIFadd{Context-Oriented Seismic Image Synthesis }\DIFaddend methodology combines a deep learning model (VAE) with a texture synthesis algorithm\DIFdelbegin \DIFdel{to generate }\DIFdelend \DIFaddbegin \DIFadd{.  It generates }\DIFaddend seismic image samples for data augmentation within a specific context mask, as outlined below. The convolutional neural network is a variational auto-encoder model (VAE) that generates salt body masks corresponding to a given salt geometry. Using the generated mask geometry, a new image is synthesized using a non-parametric texture synthesis algorithm. The proposed method focuses on context zones and on the characteristics of seismic images with salt domes. %A schema of the proposed method is presented in Algorithm.  ~\ref{alg:alg1}.
...
\begin{enumerate}
    \item \textbf{\DIFdelbegin \DIFdel{Visual }\DIFdelend \DIFaddbegin \DIFadd{Qualitative }\DIFaddend Comparison \DIFaddbegin \DIFadd{by Experts}\DIFaddend }: \DIFdelbegin \DIFdel{The authors present examples of synthesized textures, showing that the proposed method better preserves the global and local structures of the original texture, while other approaches often lose details or introduce artifacts. }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\textbf{\DIFdel{Quantitative Metrics}}%DIFAUXCMD
\DIFdel{: Objective }\DIFdelend \DIFaddbegin \DIFadd{Experts evaluate seismic samples by trying to identify the portions of salt dome structures inside them. The expert produces a mask label that will be compared with the ground truth. Quantitative }\DIFaddend metrics, such as the F1 score, are used to measure the \DIFdelbegin \DIFdel{similarity between the generated textures and the expert evaluation. 
    }\DIFdelend \DIFaddbegin \DIFadd{expert prediction and the ground truth. 
    }

    \item \textbf{\DIFadd{Comparing results with related works}}\DIFadd{:
    In “Generating Sketch-Based Synthetic Seismic Images With Generative Adversarial Networks” \mbox{%DIFAUXCMD
\cite{ref19}}\hskip0pt%DIFAUXCMD
, use the metrics mean squared error (MSE), structural similarity (SSIM) and Euclidean distance based on the Local Binary Pattern (LBP) to measure
    texture attributes between original seismic sketches and the samples synthesized by the networks.
}\DIFaddend \end{enumerate}

\DIFdelbegin \DIFdel{The qualitative evaluation of the samples was conducted through a systematic and rigorous process with the support of experts in the field. The goal was to verify the synthesized samples' quality, relevance, and reliability using the knowledge and experience of qualified professionals. Three geology and geophysics experts routinely work in seismic interpretation participated in this evaluation. The evaluation was conducted by reviewing a seismic image and selecting the regions by painting the salt bodies using a graphics computer program.
The area painted by the expert is then compared with the jig established for the corresponding saline body of the image. First, the specialist's accuracy was assessed by identifying salt bodies on twenty original images from the dataset. Then, the selection of the areas is performed on a set of thirty random synthetic images. The specialists repeat the process with the same images. In addition, experts are warned that some images are added for control without any saline body.
}%DIFDELCMD < 
...
\subsection{\DIFdelbegin \DIFdel{Obtained Experimental Results}\DIFdelend \DIFaddbegin \DIFadd{Qualitative Comparison by Experts}\DIFaddend }

\DIFdelbegin \DIFdel{To evaluate the results, the consolidated measures and their averages and standard deviations were obtained }\DIFdelend \DIFaddbegin \DIFadd{The qualitative evaluation of the samples was conducted through a systematic and rigorous process with the support of experts in the field. The goal was to verify the synthesized samples' quality, relevance, and reliability using the knowledge and experience of qualified professionals. Three geology and geophysics experts who routinely work in seismic interpretation participated in this evaluation. The evaluation was conducted by reviewing a seismic image and selecting the regions by painting the salt bodies using a graphics computer program. The area painted by the expert is then compared with the jig established for the corresponding saline body of the image. First, the specialist's accuracy was assessed by identifying salt bodies on twenty original images from the dataset. Then, the selection of the areas is performed on a set of thirty random synthetic images. The specialists repeat the process with the same images. In addition, experts are warned that some images are added for control without any saline body.
}
...
\DIFaddbegin \subsection{\DIFadd{Comparing with Related Work}}
...
\DIFdelend \DIFaddbegin \begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in, clip,keepaspectratio]{images/luciano.jpg}}]{Luciano D. Terres} \DIFaddend received the B.S. \DIFdelbegin \DIFdel{and M.S. degrees in aerospace engineering from the University of Virginia, Charlottesville, in 2001 }\DIFdelend \DIFaddbegin \DIFadd{degree in Computational Science from the Universidade Federal do Rio Grande do Sul, Porto Alegre - RS, in 1996. In 2010 received the M.S. 
}\DIFaddend and the Ph.D. degree in mechanical engineering from Drexel University, Philadelphia, PA, in 2008.
