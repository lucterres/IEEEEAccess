...
\title{Context-oriented Synthesis of Salt Domes Labeled Seismic Images}
...
The Context-Oriented Seismic Image Synthesis methodology combines a deep learning model (VAE) with a texture synthesis algorithm.  It generates seismic image samples for data augmentation within a specific context mask, as outlined below. The convolutional neural network is a variational auto-encoder model (VAE) that generates salt body masks corresponding to a given salt geometry. Using the generated mask geometry, a new image is synthesized using a non-parametric texture synthesis algorithm. The proposed method focuses on context zones and on the characteristics of seismic images with salt domes. %A schema of the proposed method is presented in Algorithm.  ~\ref{alg:alg1}.
...
\begin{enumerate}
    \item \textbf{Qualitative Comparison by Experts}: Experts evaluate seismic samples by trying to identify the portions of salt dome structures inside them. The expert produces a mask label that will be compared with the ground truth. Quantitative metrics, such as the F1 score, are used to measure the expert prediction and the ground truth. 
    
    \item \textbf{Comparing results with related works}:
    In “Generating Sketch-Based Synthetic Seismic Images With Generative Adversarial Networks” \cite{ref19}, use the metrics mean squared error (MSE), structural similarity (SSIM) and Euclidean distance based on the Local Binary Pattern (LBP) to measure
...
\subsection{Qualitative Comparison by Experts}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in, clip,keepaspectratio]{images/luciano.jpg}}]{Luciano D. Terres} received the B.S. degree in Computational Science from the Universidade Federal do Rio Grande do Sul, Porto Alegre - RS, in 1996. In 2010 received the M.S. 
and the Ph.D. degree in mechanical engineering from Drexel University, Philadelphia, PA, in 2008.
